...
User finishes → sets annotation agents.kruise.io/reuse=true (trigger) on sandbox
    ...
    → Running phase:
        ...
        → Execute cleanup via SandboxReuser interface (clean user data)
        ...
        → Cleanup success:
            → Cooldown period (--cooldown-grace-period, default 10s, phase remains Reusing)
...
3. **Execute cleanup** (includes user data cleanup + metadata restoration):
   - Clean user data via `SandboxReuser.Reuse()`.
...
The sandbox controller invokes cleanup logic through the `SandboxReuser` interface, decoupled from any specific implementation. Different scenarios can provide different implementations (e.g., via in-Pod sidecar API, exec command, or external service).

```go
// SandboxReuser handles the actual cleanup of user state inside a sandbox.
// Implementations are injected into the sandbox controller at startup.
type SandboxReuser interface {
    // Reuse triggers the cleanup of user state in the sandbox.
...
3. **Reset environment variables** — clear environment variables set during claim.
4. **Reset network state** — clear user-created network rules or connections.
5. **Health check** — verify the sandbox is in a clean, usable state.
...
2. **Phase 2 — Sandbox controller**: Implement reuse reconciliation (detect annotations → Reusing phase → call `SandboxReuser.Reuse()` → reset claim metadata via template → cooldown → Running+Ready + restore OwnerRef → Available). Add `--reuse-timeout`, `--cooldown-grace-period` flags.
