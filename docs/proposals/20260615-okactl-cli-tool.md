...
| `set image status` | `Get` | Read-only status check, supports `--wait` for polling |
...
pkg/cli/
  ...
  setimage.go          # set image sandboxset implementation + status subcommand
  setimage_test.go     # Table-driven tests (21 cases)
...
8. **`set image status` for update progress**: After running `set image sandboxset`, users can
   check the rolling update progress with `set image status NAME`. This reads the SandboxSet's
   `status.updatedReplicas` and `status.availableReplicas` fields. The `--wait` flag polls
   every 3 seconds until all replicas are updated and available.
