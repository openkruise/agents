...
Configured credentials are validated before peer listeners or outbound transports become usable. A malformed reference, unreadable Secret, missing selected value, invalid shared key, unusable certificate chain, mismatched private key, or incompatible certificate purpose fails the entire process startup. The process does not silently disable the broken protection, try another field, generate replacement material, or leave the control API running as a partially configured instance. An explicit request for protection is therefore a fail-closed contract.
...
Credentials are a startup snapshot. Secret changes, certificate renewal, or trust changes take effect only after restart. Keeping loading independent from the existing runtime TLS loader costs an extra exact Secret read at startup, but avoids coupling peer security to runtime-specific state and failure handling. There is no background retry, hot reload, or immediate revocation guarantee: a certificate that expires after startup can break later handshakes while an existing connection remains alive.
