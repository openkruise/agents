...
bash -c "$GINKGO_CMD"
retVal=$?
restartCount=$(kubectl get pod -n sandbox-system --no-headers | awk '{print $4}')
if [ "${restartCount}" -eq "0" ];then
    echo "agent-sandbox-controller has not restarted"
else
    kubectl get pod -n sandbox-system --no-headers
    echo "agent-sandbox-controller has restarted, abort!!!"
    kubectl get pod -n sandbox-system --no-headers | awk '{print $1}' | xargs kubectl logs -p -n sandbox-system
    exit 1
fi
...
if [ "$PRINT_INFO" = true ]; then
    if [ "$retVal" -ne 0 ];then
        ...
        done < <(kubectl get pods -n sandbox-system --no-headers | awk '{print $1}')
    fi
fi
