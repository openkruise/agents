...
`DeletionTimestamp`, it authoritatively deletes by ObjectKey. When the Store reports an ambiguity, the gateway adapter enqueues the
...
The Store maintains one full record per ObjectKey plus deletion fences and derived active-ID and
...
peer payload rejected at the boundary cannot quarantine an existing full record.
...
| Route Store | Atomic ID switch, same/different UID RV fencing, collision quarantine and recovery |
