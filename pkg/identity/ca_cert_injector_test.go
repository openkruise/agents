...
// mockCABundleProvider is a test mock that implements IdentityProvider
// with configurable GetProxyCABundle behavior.
type mockCABundleProvider struct {
	defaultTokenProvider
	caBundle    string
	providerErr string
}

func (m *mockCABundleProvider) GetProxyCABundle(_ context.Context, _ GetProxyCABundleRequest) (*GetProxyCABundleResponse, error) {
	if m.providerErr != "" {
		return nil, assert.AnError
	}
	return &GetProxyCABundleResponse{CABundle: m.caBundle}, nil
}
...
func TestGatewayCACertInjector_EnsureGatewayCACert(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			provider = &mockCABundleProvider{
				caBundle:    tt.proxyCABundle,
