...
func TestInitPeers(t *testing.T) {
	...
	t.Run("nil sessionManager does not panic", func(t *testing.T) {
		s := &MCPServer{
			sessionManager: nil,
		}
		...
		assert.NotPanics(t, func() {
			s.InitPeers([]string{"10.0.0.1", "10.0.0.2"})
		})
	})

	t.Run("filters empty IP strings", func(t *testing.T) {
		s := createTestMCPServerForRegisterTools()
		...
		// Should only have 3 peers (empty strings filtered)
		assert.Len(t, s.sessionManager.peers, 3)
		// Empty string should not be added
		_, emptyExists := s.sessionManager.peers[""]
		assert.False(t, emptyExists, "empty string should not be registered as peer")
	})

	t.Run("empty IP list does nothing", func(t *testing.T) {
		s := createTestMCPServerForRegisterTools()
		...
		assert.Len(t, s.sessionManager.peers, 0)
	})

	t.Run("all empty IPs results in no peers", func(t *testing.T) {
		s := createTestMCPServerForRegisterTools()
