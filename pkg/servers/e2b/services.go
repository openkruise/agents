...
func (sc *Controller) CreateSandbox(r *http.Request) (web.ApiResponse[*models.Sandbox], *web.ApiError) {
	...
	sbx, err := sc.manager.ClaimSandbox(ctx, user.ID.String(), request.TemplateID, infra.ClaimSandboxOptions{
		Modifier: func(sbx infra.Sandbox) {
			...
			annotations[models.AnnotationEnvdAccessToken] = accessToken
			route := sbx.GetRoute()
			annotations[models.AnnotationEnvdURL] = fmt.Sprintf("http://%s:%d", route.IP, models.EnvdPort)
	...
	if pool.GetAnnotations()[models.AnnotationShouldInitEnvd] == utils.True {
	...
	if request.Extensions.StorageMount.MountPath != "" {
		if err = sbx.Mount(ctx, request.Extensions.StorageMount); err != nil {
...
func (sc *Controller) DescribeSandbox(r *http.Request) (web.ApiResponse[*models.Sandbox], *web.ApiError) {
	...
	return web.ApiResponse[*models.Sandbox]{
		Body: sc.convertToE2BSandbox(sbx, sbx.GetAnnotations()[models.AnnotationEnvdAccessToken]),
	}, nil
}
