...
func (sc *Controller) CreateSandbox(r *http.Request) (web.ApiResponse[*models.Sandbox], *web.ApiError) {
	...
	opts := infra.ClaimSandboxOptions{
		...
		Modifier: func(sbx infra.Sandbox) {
			...
			if request.AutoPause {
				timeoutOptions.ShutdownTime = TimeAfterSeconds(claimStart, sc.maxTimeout)
				timeoutOptions.PauseTime = TimeAfterSeconds(claimStart, request.Timeout)
			} else {
	...
	log.Info("sandbox claimed", "id", sbx.GetSandboxID(), "sbx", klog.KObj(sbx), "totalCost", time.Since(claimStart))
