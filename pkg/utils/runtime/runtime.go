...
func GetAccessToken(sbx *agentsv1alpha1.Sandbox) string {
	token := sbx.Annotations[agentsv1alpha1.AnnotationRuntimeAccessToken]
	if token == "" {
		token = sbx.Annotations[agentsv1alpha1.AnnotationEnvdAccessToken] // legacy
	}
	return token
}
