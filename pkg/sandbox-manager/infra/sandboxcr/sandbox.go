...
var MountCommand = "/todo/sandbox-storage"

// Mount creates a dynamic mount point in Sandbox with `sandbox-storage` cli
//
// NOTE: `sandbox-storage` cli should be injected with `sandbox-runtime` and will be replaced by a built-in service of
// `sandbox-runtime`.
func (s *Sandbox) Mount(ctx context.Context, opts storage.MountOptions) error {
	log := klog.FromContext(ctx).WithValues("sandbox", klog.KObj(s.Sandbox))
	args := opts.Args.ToMountArgs()
	processConfig := &process.ProcessConfig{
		Cmd: MountCommand,
		Cwd: nil,
	}
	processConfig.Args = make([]string, 0, len(args)+5)
	processConfig.Args = append(processConfig.Args,
		"mount",
		"--mountPoint", opts.MountPath,
		"--podUID", string(s.Sandbox.Status.PodInfo.UID))
	processConfig.Args = append(processConfig.Args, args...)
...
func AsSandboxDeepCopy(sbx *agentsv1alpha1.Sandbox, cache *Cache, client sandboxclient.Interface) *Sandbox {
	sbx = sbx.DeepCopy()
	...
	return AsSandbox(sbx, cache, client)
}
