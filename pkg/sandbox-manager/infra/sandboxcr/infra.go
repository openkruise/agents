...
func (i *Infra) readClaimedSandbox(ctx context.Context, opts infra.GetSandboxOptions, lookup *claimedSandboxLookup) error {
	...
	lookup.sandbox = got
	lookup.observeRoute(i.Proxy, opts.SandboxID)
	return nil
}
...
func (i *Infra) lookupSandbox(ctx context.Context, opts infra.GetSandboxOptions) (claimedSandboxLookup, error) {
	...
	if !errors.Is(err, cache.ErrSandboxNotFound) || !lookup.observeRoute(i.Proxy, opts.SandboxID) {
		return lookup, err
	}
...
func (l *claimedSandboxLookup) observeRoute(server *proxy.Server, sandboxID string) bool {
	route, ok := server.LoadRoute(sandboxID)
	if !ok {
		return l.hasRoute
	}
	if !l.hasRoute || expectations.IsResourceVersionReallyNewer(l.route.ResourceVersion, route.ResourceVersion) {
		l.route = route
		l.hasRoute = true
	}
	return true
