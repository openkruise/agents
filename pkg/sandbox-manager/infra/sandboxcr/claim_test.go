...
func TestModifyPickedSandbox_SecurityToken(t *testing.T) {
	tests := []struct {
		...
		{
			name: "with security token options",
			opts: infra.ClaimSandboxOptions{
				SecurityToken: &config.SecurityTokenOptions{
					TokenResponse: identityprovider.TokenResponse{
						RequestID:   "req-123",
			...
			expectedAnnos: map[string]string{
				models.ExtensionKeyClaimWithSecurityIdentityProvider: `{"requestId":"req-123","accessToken":"security-access-token"}`,
			},
		},
		{
			name: "with security token including optional fields",
			opts: infra.ClaimSandboxOptions{
				SecurityToken: &config.SecurityTokenOptions{
					TokenResponse: identityprovider.TokenResponse{
						RequestID:              "req-456",
						AccessToken:            "at-456",
						RefreshToken:           "rt-456",
						SandboxClientID:        "client-789",
						AccessTokenExpiration:  "2026-12-31T23:59:59Z",
						RefreshTokenExpiration: "2027-01-31T23:59:59Z",
					},
				},
			},
		},
		{
			...
			notExpectedAnnos: []string{
				models.ExtensionKeyClaimWithSecurityIdentityProvider,
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// For the "with optional fields" case, verify JSON round-trip with all fields
			if tt.name == "with security token including optional fields" {
				raw := annotations[models.ExtensionKeyClaimWithSecurityIdentityProvider]
				require.NotEmpty(t, raw)
				var decoded config.SecurityTokenOptions
				require.NoError(t, json.Unmarshal([]byte(raw), &decoded))
				assert.Equal(t, "req-456", decoded.RequestID)
				assert.Equal(t, "at-456", decoded.AccessToken)
				assert.Equal(t, "rt-456", decoded.RefreshToken)
				assert.Equal(t, "client-789", decoded.SandboxClientID)
				assert.Equal(t, "2026-12-31T23:59:59Z", decoded.AccessTokenExpiration)
				assert.Equal(t, "2027-01-31T23:59:59Z", decoded.RefreshTokenExpiration)
			}
		})
	}
}
...
type mockIdentityProvider struct {
	issueTokenFunc    func(ctx context.Context, req identityprovider.TokenRequest) (*identityprovider.TokenResponse, error)
	propagateFunc     func(ctx context.Context, sbx *v1alpha1.Sandbox, tokenResp *identityprovider.TokenResponse) error
}

func (m *mockIdentityProvider) IssueToken(ctx context.Context, req identityprovider.TokenRequest) (*identityprovider.TokenResponse, error) {
	if m.issueTokenFunc != nil {
		return m.issueTokenFunc(ctx, req)
	}
	return &identityprovider.TokenResponse{AccessToken: uuid.NewString()}, nil
}

func (m *mockIdentityProvider) PropagateSecurityToken(ctx context.Context, sbx *v1alpha1.Sandbox, tokenResp *identityprovider.TokenResponse) error {
...
func TestTryClaimSandbox_SecurityToken(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockProvider: &mockIdentityProvider{
				issueTokenFunc: func(ctx context.Context, req identityprovider.TokenRequest) (*identityprovider.TokenResponse, error) {
					return &identityprovider.TokenResponse{AccessToken: "secure-token-123"}, nil
				},
				propagateFunc: func(ctx context.Context, sbx *v1alpha1.Sandbox, tokenResp *identityprovider.TokenResponse) error {
					return nil
				},
			...
			postCheck: func(t *testing.T, sbx infra.Sandbox, metrics infra.ClaimMetrics) {
				// Access token should be upgraded to identity_provider type
				annotations := sbx.GetAnnotations()
				assert.Equal(t, "secure-token-123", annotations[v1alpha1.AnnotationRuntimeAccessToken])
				// SecurityToken metrics should be recorded
				assert.Greater(t, metrics.SecurityToken, time.Duration(0))
				// SecurityToken annotation should be set
				raw := annotations[models.ExtensionKeyClaimWithSecurityIdentityProvider]
				assert.NotEmpty(t, raw)
				var decoded config.SecurityTokenOptions
				require.NoError(t, json.Unmarshal([]byte(raw), &decoded))
				assert.Equal(t, "secure-token-123", decoded.AccessToken)
			},
		},
		{
			...
			postCheck: func(t *testing.T, sbx infra.Sandbox, metrics infra.ClaimMetrics) {
				// Original UUID token should be preserved
				annotations := sbx.GetAnnotations()
				assert.Equal(t, "original-uuid-token", annotations[v1alpha1.AnnotationRuntimeAccessToken])
				// SecurityToken annotation should still be set (empty token)
				raw := annotations[models.ExtensionKeyClaimWithSecurityIdentityProvider]
				assert.NotEmpty(t, raw)
			},
		},
		{
			...
			mockProvider: &mockIdentityProvider{
				issueTokenFunc: func(ctx context.Context, req identityprovider.TokenRequest) (*identityprovider.TokenResponse, error) {
					return &identityprovider.TokenResponse{AccessToken: "secure-token-456"}, nil
				},
				propagateFunc: func(ctx context.Context, sbx *v1alpha1.Sandbox, tokenResp *identityprovider.TokenResponse) error {
		...
		},
		{
			...
			postCheck: func(t *testing.T, sbx infra.Sandbox, metrics infra.ClaimMetrics) {
				...
				assert.Empty(t, annotations[models.ExtensionKeyClaimWithSecurityIdentityProvider])
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			origProvider := identityprovider.DefaultProvider
			identityprovider.DefaultProvider = tt.mockProvider
			t.Cleanup(func() { identityprovider.DefaultProvider = origProvider })
