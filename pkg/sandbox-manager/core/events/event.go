...
//
// NOTE: a Pod is required for the event.
//
...
func (e *Eventer) Trigger(evt Event) int32 {
	...
	if evt.Sandbox == nil {
		log.Info("event ignored for pod is nil")
		return 0
	}
	if evt.Sandbox.GetDeletionTimestamp() != nil && evt.Type != consts.SandboxKill {
		log.Info("non SandboxKill event ignored for pod is being deleted", "type", evt.Type)
		return 0
	}
...
func (e *Eventer) next(evt Event) {
	...
	case consts.SandboxCreated:
		...
		evt.Message = fmt.Sprintf("auto triggered for Pod %s/%s is ready to be claimed as a sandbox",
...
func (e *Eventer) OnSandboxDelete(sbx infra.Sandbox) {
	defer func() {
		// 以防万一 Pod 删除时，等待执行的事件发生 Panic
		if r := recover(); r != nil {
			klog.InfoS("Panic occurred in event handler", "pod", klog.KObj(sbx), "reason", r)
		}
	}()
