...
import (
	...
	models2 "github.com/openkruise/agents/pkg/sandbox-manager/controllers/e2b/models"
...
func (sc *Controller) CreateSandbox(r *http.Request) (web.ApiResponse[*models2.Sandbox], *web.ApiError) {
	ctx := r.Context()
	log := klog.FromContext(ctx)
	start := time.Now()
	user := GetUserFromContext(ctx)
	if user == nil {
		return web.ApiResponse[*models2.Sandbox]{}, &web.ApiError{
			Code:    http.StatusUnauthorized,
			Message: "User is empty",
		}
	}
	var request models2.NewSandboxRequest
	...
	for k := range request.Metadata {
		...
		if err := utils2.ValidatedCustomLabelKey(k); err != nil {
			return web.ApiResponse[*models2.Sandbox]{}, &web.ApiError{
				Code:    http.StatusBadRequest,
	...
	template := pool.GetTemplate()
	var shouldInitEnvd bool
...
func (sc *Controller) ListSandboxes(r *http.Request) (web.ApiResponse[[]*models2.Sandbox], *web.ApiError) {
	var request models2.ListSandboxesRequest
	...
	for key, values := range r.URL.Query() {
		...
		case "state":
			state := values[0]
			if state != models2.SandboxStateRunning && state != models2.SandboxStatePaused {
				return web.ApiResponse[[]*models2.Sandbox]{}, &web.ApiError{
					Code:    http.StatusBadRequest,
	...
	sandboxes := make([]*models2.Sandbox, 0, len(pods))
...
func (sc *Controller) DescribeSandbox(r *http.Request) (web.ApiResponse[*models2.Sandbox], *web.ApiError) {
...
func (sc *Controller) SetSandboxTimeout(r *http.Request) (web.ApiResponse[struct{}], *web.ApiError) {
	...
	log := klog.FromContext(ctx)
	id := r.PathValue("sandboxID")
	var request models2.SetTimeoutRequest
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil {
		return web.ApiResponse[struct{}]{}, &web.ApiError{
			Message: err.Error(),
		}
...
func (sc *Controller) BrowserUse(r *http.Request) (web.ApiResponse[*browserHandShake], *web.ApiError) {
	...
	resp, err := sbx.Request(r, "/json/version", models2.CDPPort)
	if err != nil {
		return web.ApiResponse[*browserHandShake]{}, &web.ApiError{
			Message: fmt.Sprintf("Failed to proxy request to sandbox port %d: %v", models2.CDPPort, err),
		}
	}
	...
	h.WebSocketDebuggerURL = browserWebSocketReplacer.ReplaceAllString(h.WebSocketDebuggerURL,
		fmt.Sprintf("wss://%s", utils.GetSandboxAddress(sandboxID, sc.domain, models2.CDPPort)))
...
func (sc *Controller) Debug(_ *http.Request) (web.ApiResponse[core.DebugInfo], *web.ApiError) {
	return web.ApiResponse[core.DebugInfo]{
		Body: sc.manager.GetDebugInfo(),
	}, nil
}

func (sc *Controller) ListAPIKeys(r *http.Request) (web.ApiResponse[[]*models2.TeamAPIKey], *web.ApiError) {
	ctx := r.Context()
	user := GetUserFromContext(ctx)
	if user == nil {
		return web.ApiResponse[[]*models2.TeamAPIKey]{}, &web.ApiError{
			Message: "User not found",
		}
	}
	apiKeys := sc.keys.ListByOwner(user.ID)

	return web.ApiResponse[[]*models2.TeamAPIKey]{
		Body: apiKeys,
	}, nil
}

func (sc *Controller) CreateAPIKey(r *http.Request) (web.ApiResponse[*models2.CreatedTeamAPIKey], *web.ApiError) {
	var request models2.NewTeamAPIKey
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil {
		return web.ApiResponse[*models2.CreatedTeamAPIKey]{}, &web.ApiError{
			Message: err.Error(),
		}
	}
	...
	return web.ApiResponse[*models2.CreatedTeamAPIKey]{
		Code: http.StatusCreated,
		Body: createdAPIKey,
	}, nil
}
