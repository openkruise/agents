...
// IsSandboxPaused checks the informer cache whether the sandbox is actually
// paused (Spec.Paused=true). This is needed because the gateway controller's
// route registry may have a stale "running" state when the sandbox controller
// auto-pauses a sandbox without syncing routes to peers.
func (w *Waker) IsSandboxPaused(ctx context.Context, namespace, name string) bool {
	if w == nil {
		return false
	}
	cli := w.cache.GetClient()
