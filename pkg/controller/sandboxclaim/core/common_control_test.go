		...
		},
		{
			...
			claim: &agentsv1alpha1.SandboxClaim{
				...
				Spec: agentsv1alpha1.SandboxClaimSpec{
					...
					Labels: map[string]string{
						"team":                               "blue",
						agentsv1alpha1.LabelSandboxPool:      "other-pool",
			...
			validate: func(t *testing.T, opts infra.ClaimSandboxOptions) {
				...
				assert.Equal(t, "real-pool", podLabels[agentsv1alpha1.LabelSandboxPool], "pool label on the pod template must survive the claim")

				// A reserved key recorded here would be deleted from the
				// Sandbox by resetSandboxForPool when it is recycled.
