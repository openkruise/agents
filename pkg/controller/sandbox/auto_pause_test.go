...
func TestEvaluateResumeSchedule(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Seed a time recorded by an earlier reconcile. No path may leave it
			// behind: a recorded resume must not outlive the decision that
			// produced it, or the status keeps advertising a resume that is no
			// longer going to happen.
			stale := metav1.NewTime(now.Time.Add(-10 * time.Minute))
			tt.newStatus.Schedules = []agentsv1alpha1.Schedule{
			...
			if tt.wantNil {
				assert.Nil(t, result)
				assert.Nil(t, sched.NextResumeTime, "expected the stale resume time to be cleared")
				return
			}
...
func TestEvaluatePauseSchedule(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Seed a time recorded by an earlier reconcile. Every path has to
			// clear it, including the fail-closed ones: once the probe stops
			// reporting idle, a leftover NextPauseTime keeps advertising a pause
			...
			if tt.wantNil {
				assert.Nil(t, got)
				assert.Nil(t, sched.NextPauseTime, "expected the stale pause time to be cleared")
				return
			}
			...
			assert.Equal(t, got, sched.NextPauseTime, "expected the recorded pause time to match the decision")
		})
	}
}
...
func TestHandleAutoPause_PausedRecordsNoPauseSchedule(t *testing.T) {
	...
	box := makeProbeSandbox("paused-idle-condition", agentsv1alpha1.SandboxPaused, func(b *agentsv1alpha1.Sandbox) {
		...
		// longer exists. Its transition time keeps receding into the past, and any
		// pause time derived from it is a fossil that the status would keep
		// advertising for as long as the sandbox stays paused.
		b.Status.Conditions = []metav1.Condition{
			{
				Type:               condType,
				Status:             metav1.ConditionTrue,
...
func TestHandleAutoPause_BranchesOnNewStatusPhase(t *testing.T) {
	...
	resumeCondType := agentsv1alpha1.ProbeConditionPrefix + "schedule"
	pastTime := time.Now().Add(-1 * time.Minute)
	// Status.Phase is what the previous reconcile persisted; newStatus.Phase is
	// the phase this reconcile is handling. The pause already took effect, so the
	// resume decision has to be made now rather than one reconcile later.
	box := makeProbeSandbox("stale-status-phase", agentsv1alpha1.SandboxRunning, func(b *agentsv1alpha1.Sandbox) {
		b.Spec.Paused = true
		b.Spec.AutoPausePolicy.Resume = &agentsv1alpha1.ResumePolicy{
			WhenProbedScheduleTime: &agentsv1alpha1.ProbedScheduleTimeRule{
				Probe:      "schedule",
				TimeFormat: "unix",
				LeadTime:   &metav1.Duration{Duration: 0},
			},
		}
		b.Status.Conditions = []metav1.Condition{
			{
	...
	updated := &agentsv1alpha1.Sandbox{}
	require.NoError(t, fakeClient.Get(context.Background(), client.ObjectKeyFromObject(box), updated))
	assert.False(t, updated.Spec.Paused, "the resume branch must run for the phase this reconcile is handling")
}
...
func TestPauseSandbox(t *testing.T) {
	...
	expired := metav1.NewTime(now.Time.Add(-1 * time.Minute))

	tests := []struct {
		name string
		// retention is the value of the paused-retention annotation; empty means
		// the annotation is absent.
		retention string
		paused    bool
		// shutdownTime is the ShutdownTime the sandbox carries before the pause.
		shutdownTime *metav1.Time
		// wantExtendBy, when non-nil, is the retention the pause must add to now.
		// When nil, ShutdownTime must be left exactly as it was.
		wantExtendBy *time.Duration
	}{
		{
			// Without the annotation the sandbox is not under a retention policy,
			// so ShutdownTime keeps whatever the API layer computed.
			name:         "no retention annotation - shutdown time untouched",
			shutdownTime: &expired,
		},
		{
			// The pause is what buys the retention window. Leaving the expired
			// ShutdownTime behind lets the next reconcile delete the sandbox right
			// after pausing it, discarding the whole window.
			name:         "retention annotation - expired shutdown time extended",
			retention:    "30m",
			shutdownTime: &expired,
			wantExtendBy: ptr.To(30 * time.Minute),
		},
		{
			name:      "retention annotation but no shutdown time - nothing to extend",
			retention: "30m",
		},
		{
			// An unparsable annotation still signals retention intent, so it falls
			// back to the built-in retention rather than dropping the window.
			name:         "invalid retention annotation - falls back to default",
			retention:    "not-a-duration",
			shutdownTime: &expired,
			wantExtendBy: ptr.To(timeoututils.ForeverReservePausedSandboxDuration),
		},
		{
			name:         "already paused - no-op",
			retention:    "30m",
			paused:       true,
			shutdownTime: &expired,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			box := makeProbeSandbox("pause-sandbox", agentsv1alpha1.SandboxRunning, func(b *agentsv1alpha1.Sandbox) {
				b.Spec.Paused = tt.paused
				b.Spec.ShutdownTime = tt.shutdownTime.DeepCopy()
				if tt.retention != "" {
					b.Annotations = map[string]string{
						agentsv1alpha1.AnnotationReservePausedSandboxDuration: tt.retention,
					}
				}
			})
			r, fakeClient := newAutoPauseReconciler(t, scheme, box)

			require.NoError(t, r.pauseSandbox(context.Background(), box, now))

			updated := &agentsv1alpha1.Sandbox{}
			require.NoError(t, fakeClient.Get(context.Background(), client.ObjectKeyFromObject(box), updated))
			assert.True(t, box.Spec.Paused, "local copy must reflect the pause")
			assert.True(t, updated.Spec.Paused, "persisted object must reflect the pause")

			if tt.wantExtendBy == nil {
				assert.Equal(t, tt.shutdownTime, box.Spec.ShutdownTime, "ShutdownTime must not change")
				assert.Equal(t, tt.shutdownTime, updated.Spec.ShutdownTime, "persisted ShutdownTime must not change")
				return
			}
			want := metav1.NewTime(pausedretention.PausedShutdownTime(now.Time, *tt.wantExtendBy))
			require.NotNil(t, box.Spec.ShutdownTime)
			assert.True(t, want.Equal(box.Spec.ShutdownTime),
				"expected ShutdownTime %v, got %v", want, box.Spec.ShutdownTime)
			require.NotNil(t, updated.Spec.ShutdownTime)
			assert.True(t, want.Equal(updated.Spec.ShutdownTime),
				"expected persisted ShutdownTime %v, got %v", want, updated.Spec.ShutdownTime)
			// PauseTime moves with ShutdownTime so a later connect/resume still sees
			// the sandbox as auto-pause managed rather than as plainly timed out.
			require.NotNil(t, updated.Spec.PauseTime)
			assert.True(t, want.Equal(updated.Spec.PauseTime),
				"expected persisted PauseTime %v, got %v", want, updated.Spec.PauseTime)
		})
	}
}
