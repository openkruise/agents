...
func TestSandboxReconcile(t *testing.T) {
	...
	for _, cs := range cases {
		t.Run(cs.name, func(t *testing.T) {
			...
			// check condition
			for i := range expectedBox.Status.Conditions {
				expectCond := &expectedBox.Status.Conditions[i]
				gotCond := utils.GetSandboxCondition(&newBox.Status, expectCond.Type)
				if gotCond == nil {
					...
					if !reflect.DeepEqual(gotCond, expectCond) {
						t.Errorf("sandbox condition not equal, expect: %v, got: %v", expectCond, gotCond)
					}
				}
			}

			if objectDiff := diff.ObjectReflectDiff(expectedBox, newBox); objectDiff != "<no diffs>" {
				t.Fatalf("sandbox diff: %s", objectDiff)
			}
			...
			if objectDiff := diff.ObjectReflectDiff(expectedPod, newPod); objectDiff != "<no diffs>" {
				t.Fatalf("pod diff: %s", objectDiff)
			}
		})
	}
}
