...
func TestCommonControl_EnsureSandboxUpdated_AnnotationRemoval(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = clientgoscheme.AddToScheme(scheme)
	_ = agentsv1alpha1.AddToScheme(scheme)

	// Test: inplace update done with enable-inplace-update annotation
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-sandbox",
			Namespace: "default",
			Labels:    map[string]string{}, // no template hash label -> handleInplaceUpdate returns done=true
		},
		Spec: corev1.PodSpec{
			NodeName: "node-1",
		},
	...
	box := &agentsv1alpha1.Sandbox{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "default",
			Annotations: map[string]string{
				agentsv1alpha1.SandboxAnnotationEnableInplaceUpdate: "true",
			},
		},
		Spec: agentsv1alpha1.SandboxSpec{
			EmbeddedSandboxTemplate: agentsv1alpha1.EmbeddedSandboxTemplate{
	...
	fakeClient := fake.NewClientBuilder().WithScheme(scheme).WithStatusSubresource(box).WithObjects(pod, box).Build()
	control := &commonControl{
		Client:               fakeClient,
		recorder:             record.NewFakeRecorder(10),
		inplaceUpdateControl: inplaceupdate.NewInPlaceUpdateControl(fakeClient, inplaceupdate.DefaultGeneratePatchBodyFunc),
	...
	// Verify status was synced from pod
	if newStatus.NodeName != "node-1" {
		t.Errorf("Expected NodeName 'node-1', got %s", newStatus.NodeName)
	}
	if newStatus.SandboxIp != "10.0.0.1" {
		t.Errorf("Expected SandboxIp '10.0.0.1', got %s", newStatus.SandboxIp)
	}
}
