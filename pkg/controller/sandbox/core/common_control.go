...
func (r *commonControl) EnsureSandboxUpdated(ctx context.Context, args EnsureFuncArgs) error {
	...
	// After inplace update completes, remove the enable-inplace-update annotation
	// to prevent re-triggering inplace update on subsequent reconcile loops.
	if box.Annotations[agentsv1alpha1.SandboxAnnotationEnableInplaceUpdate] == "true" {
		patchBody := fmt.Sprintf(`{"metadata":{"annotations":{"%s":null}}}`, agentsv1alpha1.SandboxAnnotationEnableInplaceUpdate)
		clone := box.DeepCopy()
		err = r.Status().Patch(ctx, clone, client.RawPatch(types.StrategicMergePatchType, []byte(patchBody)))
		if err != nil {
			klog.ErrorS(err, "failed to remove enable-inplace-update annotation", "sandbox", klog.KObj(box))
			return err
		}
		klog.InfoS("removed enable-inplace-update annotation", "sandbox", klog.KObj(box))
		box = clone
	}
	syncSandboxStatusFromPod(pod, newStatus)
	return nil
