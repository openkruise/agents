...
type GlobalTrafficPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*apiv1alpha1.GlobalTrafficPolicy, err error)
	// Get retrieves the GlobalTrafficPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*apiv1alpha1.GlobalTrafficPolicy, error)
...
type globalTrafficPolicyLister struct {
	listers.ResourceIndexer[*apiv1alpha1.GlobalTrafficPolicy]
}
...
func NewGlobalTrafficPolicyLister(indexer cache.Indexer) GlobalTrafficPolicyLister {
	return &globalTrafficPolicyLister{listers.New[*apiv1alpha1.GlobalTrafficPolicy](indexer, apiv1alpha1.Resource("globaltrafficpolicy"))}
}
