...
var _ = Describe("Sandbox Upgrade Lifecycle", func() {
	...
	BeforeEach(func() {
		...
		initialImage = "mirrors-ssl.aliyuncs.com/centos:7"
		updateImage = "mirrors-ssl.aliyuncs.com/centos:8"
		failedImage = "mirrors-ssl.aliyuncs.com/nginx:alpine3.20"
	})
	...
	waitForRunning := func(sandbox *agentsv1alpha1.Sandbox) {
		...
		}, time.Minute*2, time.Millisecond*500).Should(Equal(agentsv1alpha1.SandboxRunning))
	}
	...
	It("should fail preUpgrade hook and block upgrade", func() {
		...
		latest.Spec.Lifecycle.PreUpgrade.Script = "ls /"
		Expect(updateSandboxSpec(ctx, latest)).To(Succeed())
	...
	It("should stay upgrading when new pod fails to start", func() {
		...
		By("Fixing upgrade by changing to a valid compatible image")
		triggerUpgrade(sandbox, updateImage)
	...
	It("should fail postUpgrade hook and block completion", func() {
		...
		latest.Spec.Lifecycle.PostUpgrade.Script = "exit 0"
		Expect(updateSandboxSpec(ctx, latest)).To(Succeed())
