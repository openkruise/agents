...
var _ = Describe("SandboxUpdateOps E2E", func() {
	...
	Context("Batch Upgrade", func() {
		...
		It("should delete ops promptly even when another ops is actively updating in the same namespace", func() {
			labelA := fmt.Sprintf("batch-blockA-%d", time.Now().UnixNano())
			labelB := fmt.Sprintf("batch-blockB-%d", time.Now().UnixNano())
			...
			for i := 0; i < 2; i++ {
				sbxA[i] = newOpsSandbox(
					fmt.Sprintf("ops-blockA-%s-%d", labelA[:10], i),
					labelA, nil, nil,
				)
				Expect(k8sClient.Create(ctx, sbxA[i])).To(Succeed())
			}
			sbxB := newOpsSandbox(fmt.Sprintf("ops-blockB-%s-0", labelB[:10]), labelB, nil, nil)
			...
			opsB := &agentsv1alpha1.SandboxUpdateOps{
				ObjectMeta: metav1.ObjectMeta{
					Name:      fmt.Sprintf("ops-blockB-%s", labelB[:10]),
					Namespace: namespace,
				},
				Spec: agentsv1alpha1.SandboxUpdateOpsSpec{
					Selector: &metav1.LabelSelector{
			...
			opsA := &agentsv1alpha1.SandboxUpdateOps{
				ObjectMeta: metav1.ObjectMeta{
					Name:      fmt.Sprintf("ops-blockA-%s", labelA[:10]),
					Namespace: namespace,
				},
				Spec: agentsv1alpha1.SandboxUpdateOpsSpec{
					Selector: &metav1.LabelSelector{
