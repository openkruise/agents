...
// newSandboxUpdateOpsTestManager creates a real controller-runtime Manager
...
func newSandboxUpdateOpsTestManager(t *testing.T) ctrl.Manager {
	t.Helper()
...
// sandboxupdateops controller with a real controller-runtime manager without
// error. The manager is never started, so no apiserver is needed.
func TestReconciler_SetupWithManager(t *testing.T) {
	mgr := newSandboxUpdateOpsTestManager(t)
	r := &Reconciler{
